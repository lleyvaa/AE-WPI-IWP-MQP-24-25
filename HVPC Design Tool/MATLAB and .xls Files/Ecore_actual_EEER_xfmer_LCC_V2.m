function y = Ecore_actual_EEER_xfmer_LCC_Copy(raw, raw1 , raw2 , raw3, raw4 , raw5,...
...
NoMat = m1-1;
CoreMatIndexSweep = ones(size(1:1:NoMat));
...
[Po, fs ,Vppeak, Vspeak , Vinsulation_max , matno_record , ShuffleXcoreIndex ,Np, Mlp, Mls] = ndgrid(Po_range,...
    ...
    MinPriWinding:IncreNp:MaxPriWinding, 1, 1:IncreMls:MaxMls);
...
Pcore = XCorePloss(ShuffleXcoreIndex);
...
Pcore = repelem(Pcore(UniqueRowIdcs), ColDuplicate);
...
if (isempty(Po))
    ...
    Pcore = repelem(Pcore, [MaxPriNstrands - MinPriNstrands + 1]);
    ...
    Pcore = repelem(Pcore, [MaxSecNstrands - MinSecNstrands + 1]);
